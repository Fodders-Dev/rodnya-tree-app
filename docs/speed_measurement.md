...
**Рекомендация реализована (13.09).** `tool/prod_route_smoke.mjs` больше не
создаёт и не удаляет три персоны на каждый прогон: `ensurePersonFixture`
находит в смоук-дереве постоянные фикстуры `Smoke Relative` / `Smoke Invite` /
`Smoke Claim` по точному имени и маркеру описания (чтобы никогда не принять за
фикстуру настоящего человека) и создаёт их только один раз; одноразовые
фикстуры прошлых прогонов с меткой времени в имени подметаются best-effort.
Удаление — только по явному `--purge-fixtures` / `RODNYA_SMOKE_PURGE_FIXTURES=1`;
`--keep-fixtures` оставлен как совместимый no-op. Проверено локально тремя
прогонами против бэкенда на копии блоба: создание → переиспользование тех же
id + подметание подложенной старой фикстуры → purge. Эффект на проде: минус
12 надгробий графа и 12 строк корзины в сутки; после первого прогона
production-watch в смоук-дереве останутся три постоянные персоны.
