...
**Находка вне SPEED-15 — зеркало графа (уточнено в SPEED-16 ниже).**
`graphPersons` — 434 КБ (28 % блоба) при 531 записи на 159 живых `persons`:
392 надгробия (`deletedAt`, все не старше 30 дней) и одна запись с `userId`,
к которой привязано 1364 уникальных `legacyPersonIds`, ни один из которых
больше не существует в `persons` (53 КБ на одну запись). Надгробия — НЕ
утечка: джоб Phase 3.6 (`hard-delete-job.js`) на проде включён и сметает их
через 30 дней; это скользящее окно мусора от смоука production-watch.
Реальных проблем две, обе закрыты в SPEED-16: сметание корзины джобом шло по
блоб-массиву, который SPEED-15 опустошил (таблица не чистилась), и
`legacyPersonIds` никогда не обрезались.
...
### Прод после деплоя SPEED-16 (12.09.2026)

Деплой `8ae1499f` (run 34623116537) поднялся 11.09 в 19:38 по времени сервера;
обрезка живёт в памяти процесса с буста, а в JSONB попадает первой же
записью блоба — она случилась на версии 516 (до деплоя 507), замер тем же
скриптом (`JSON.stringify`, только счётчики и размеры):

| метрика | до (v507) | после (v516) |
|---|---|---|
| `graphPersons`, КБ | 438,2 | 373,2 (−65 КБ, −15 %) |
| записей `graphPersons` | 537 | 540 (+3 — обычные надгробия смоука) |
| legacy-id у самой большой записи | 1368 | 3 |
| `legacyPersonIds` суммарно | 1928 | 160 (−1768 мёртвых) |
| блоб компактно, КБ | 1532,0 | 1467,0 |

Совпадает с оценкой на копии (−15,5 %, 1273 → 3). Ошибок в журнале с
...
**Рекомендация реализована (13.09, уточнено после первого живого прогона).**
`tool/prod_route_smoke.mjs`: фикстура relative-details `Smoke Relative` стала
постоянной — `ensurePersonFixture` находит её в смоук-дереве по точному имени
и маркеру описания (чтобы никогда не принять за фикстуру настоящего человека)
и создаёт только один раз. Invite/claim-фикстуры остались одноразовыми, и это
по природе, а не по лени: первый живой прогон (веб-деплой `45d0afeb`,
12.09 21:30) показал, что авторизованный партнёр их поглощает — принял
приглашение (стал персоной `Smoke Invite`), затем claim слил её в `Smoke Claim`
(`person.merged`), и «переиспользование» проверяло бы уже принятое
приглашение вместо живого сценария. Они создаются свежими и удаляются в конце
прогона; одноразовые фикстуры прошлых прогонов подметаются best-effort (первый
прогон убрал 38 штук, копившихся с апреля из-за падавшей очистки). Удаление
постоянной — только по `--purge-fixtures` / `RODNYA_SMOKE_PURGE_FIXTURES=1`.
Проверено локально тремя прогонами против бэкенда на копии блоба (создание →
переиспользование того же id + подметание → purge). Эффект: 8 надгробий и
строк корзины в сутки вместо 12 и ноль накопления остатков.
