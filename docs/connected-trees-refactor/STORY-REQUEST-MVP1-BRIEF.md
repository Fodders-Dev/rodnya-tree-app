...
| `story_request_answered` | инициатору | «{Имя адресата} ответил(а) на ваш вопрос» / первые 80 символов вопроса | `{requestId, treeId, personId, articleBlockId}` |
| `story_request_declined` | инициатору | «{Имя} пока не готов(а) ответить» / вопрос | `{requestId, treeId, personId}` |
...
| `story_request_revoked` | адресату | «{Имя} отозвал(а) вопрос» / вопрос | `{requestId}` |

Копирайт продуктового качества, обращение к пользователю на «вы» в
уведомлениях, как в остальных типах; род глагола — по `gender` пользователя,
если есть, иначе нейтральная форма («ответил(а)» не показывать — выбрать
формулировку без рода: «Ответ на ваш вопрос от {Имя}»).
