...
* **13.09 — «Спросить историю» MVP-1.1** (`714ca78e`, OTA 1.0.47+55): в шаге
  «Кого спросить» теперь и участники семьи без своей персоны в дереве —
  резолвинг семьи по `treeId` (`listMySemya`/`listMembershipsForSemya`), мёрдж
  вынесен в тестируемую `buildStoryAskTargets`; бэкенд уже принимал таких
  адресатов. Flutter 1582.
* **13.09 — «Спросить историю» MVP-1 в проде** (волна из двух sonnet-агентов,
  бэкенд `035d045b`, клиент `daa7c845`, слияние по «го»): сущность
  `storyRequests` в блобе (pending → answered / declined / expired 30 дн /
  revoked), шесть маршрутов `/v1/story-requests*` по образцу kinship-checks,
  пять уведомлений, ответ голосом/текстом/фото ложится блоком в статью героя
  с аддитивным полем `source` (подпись «Ответ на вопрос: {Имя}, {дата}»);
  право на ответ даёт сам запрос (инициатору нужно право редактировать
  персону). Клиент: capability `StoryRequestCapableFamilyTreeService`, шаг
  «Кого спросить» в листе, экран ответа voice-first (микрофон 72dp),
  маппинг уведомлений, строка статуса «Ждём ответа: … · Отозвать».
  Живая проверка на эмуляторе с двумя аккаунтами против локального бэкенда:
  вопрос → уведомление → голосовой ответ 11 с → блок с плеером и подписью →
  «делится историей» у инициатора; отзыв. По итогам проверки: правило дубля
  сужено до «тот же вопрос той же паре», заголовки уведомлений и подписи
  без склонения имён. Тесты: бэкенд 822 (+28), Flutter 1576.
  Ограничение MVP-1 (MVP-1.1): в чипы «Кого спросить» попадают только
  участники со своей персоной в дереве (клиентская модель дерева не знает
  `semyaId`); бэкенд принимает любого участника семьи. Запись стартует
  вторым тапом («Начать запись» в существующем листе) — полировка позже.
  Дальше: MVP-2 — гостевая ссылка без аккаунта и напоминание на 7-й день.
* **13.09 — чистка апрельского тестового мусора на проде** (по «го»): 21 аккаунт
  `codex-web-*` от 23.04 (след: 9 пустых чатов, 5 уведомлений, 21 сессия, 0
  деревьев/персон/семей) удалены штатным `store.deleteUser` одноразовым
  скриптом при остановленном бэкенде (простой ~10 с); вместе с ними ушли 5
  «активных» с 23.04 зомби-звонков. Итог: users 87 → 66, calls 5 → 0, chats
  42 → 33, auth_sessions 188 → 167, notifications 313 → 308, ошибок нет. Дамп
  до чистки — `/opt/rodnya/backups/manual/pre-april-cleanup-20260913-182513.dump`.
  Заодно убраны бэкап-таблицы SPEED-6/7/8b (дампы там же).
* **11.09 — SPEED-16 в проде** (`8ae1499f`, sonnet-агент, слияние по «го»):
  ...
  окно мусора смоука production-watch (12 в сутки); 13.09 фикстура
  relative-details смоука постоянная (`ensurePersonFixture`), invite/claim —
  одноразовые по природе (партнёр их поглощает): 8 надгробий в сутки вместо
  12, остатки прошлых прогонов подметаются (первый прогон убрал 38). pg-mem молча игнорирует `LIMIT` внутри
...
* ~~**Доставка приглашений** (email/SMS-провайдер)~~ — отложено 13.09,
  платный канал (DECISIONS 2026-09-13); тёплый email при наличии адреса уже есть.
* **«Спросить историю» MVP-2**: гостевая ссылка `/story/{token}` без аккаунта
  (галочка согласия), напоминание на 7-й день (нужен планировщик с доступом к
  пушам), видео-ответ (блок `video` в статье). См. STORY-REQUEST-SCENARIO.md §5.
