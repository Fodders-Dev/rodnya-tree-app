...
* **«Спросить историю»**: сценарий реализации написан 13.09 —
  [STORY-REQUEST-SCENARIO.md](STORY-REQUEST-SCENARIO.md) поверх Phase D §3.3
  (StoryRequest по образцу kinship-checks, ответ = блок статьи, гостевая
  ссылка без аккаунта); ждёт 5 решений из §6, затем MVP-1 волной агентов.
