...
* **11.09 — SPEED-16 в проде** (`8ae1499f`, sonnet-агент, слияние по «го»):
  ...
  окно мусора смоука production-watch (12 в сутки); 13.09 фикстуры смоука
  сделаны постоянными (`ensurePersonFixture`, удаление только по
  `--purge-fixtures`) — мусор больше не производится. pg-mem молча игнорирует `LIMIT` внутри
